...
export default {
  data() {
    return {
      nav: ["component", "API", "extUI", "template"],
      isPC: false,
			active: false,
			hasLeftWin: false,
			leftWinActive: ''
  ...
  unmounted() {
    isPcObserver && isPcObserver.disconnect();
    isPhoneObserver && isPhoneObserver.disconnect();
  },
  watch: {
    isPC: {
      immediate: true,
